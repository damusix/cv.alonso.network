...
- [`assets/js/exports.js`](../../assets/js/exports.js) — `exportCV()` reads `loadSavedData()`/`loadSavedStyles()`, builds a `.cvml` file with `[cv-data js]` and `[cv-styles]` tagged sections, and triggers a browser download; `importCV()` opens a file picker, parses the two sections via regex, evaluates the JS section with `new Function(cvCode)`, validates the result against `CVDataSchema`, saves it, applies imported styles via `applyStyles()` from [`assets/js/styles.js`](../../assets/js/styles.js), and updates the open Monaco editor if present

## Coupling

- [`assets/js/editor.js`](../../assets/js/editor.js) (editor-shell domain) imports `cvData`, `defaultMessage`, `CVDataSchema`, `renderCV`, and most of [`assets/js/storage.js`](../../assets/js/storage.js)'s exports directly — `applyChanges()` and `resetData()` in editor.js are the only callers that run the full validate-then-render pipeline for user JS input.
- [`assets/js/exports.js`](../../assets/js/exports.js) imports `applyStyles`/`getCurrentStyles` from [`assets/js/styles.js`](../../assets/js/styles.js) (styles domain) and `getEditorMode`/`getEditor` from [`assets/js/editor.js`](../../assets/js/editor.js) (editor-shell domain) — CVML import/export touches all three domains.
